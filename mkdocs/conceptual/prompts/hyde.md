...
Similarity search is the task of comparing text [*embeddings*](../language_models/llm.md), i.e. the vector representation of documents,
to find similarities between them, e.g. during document question-answering. 

As such, HyDE technique consists in asking the [language model](../language_models/llm.md) to generate a hypothetical answer that is used as query for the similarity search task.
...
Namely, a method that consists in providing the [language model](../language_models/llm.md) a few examples in the prompt to get more accurate answers.
