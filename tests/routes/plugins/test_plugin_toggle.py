def _get_installed(client, admin_headers):
...
def test_toggle_non_existent_plugin(client, just_installed_plugin, admin_headers):
    response = client.put("/plugins/no_plugin/toggle", headers=admin_headers)
...
def test_deactivate_plugin(client, just_installed_plugin, admin_headers):
    ...
    installed = _get_installed(client, admin_headers)
...
def test_reactivate_plugin(client, just_installed_plugin, admin_headers):
    ...
    client.put("/plugins/mock_plugin/toggle", headers=admin_headers)
    response = client.put("/plugins/mock_plugin/toggle", headers=admin_headers)
    assert response.status_code == 200
