...
# Code Constants
PLUGINS_URL = "https://plugins.cheshirecat.ai/plugins?page=1&page_size=7000"
GITHUB_API_BASE = "https://api.github.com/repos"
ADMITTED_MIME_TYPES = ["application/zip", 'application/x-tar']

def get_ccat_instance(request: Request):
    """Retrieve the ccat instance from the request."""
    return request.app.state.ccat

async def get_registry_list():
    try:
        response = await requests.get(PLUGINS_URL)
...
async def list_available_plugins(request: Request) -> Dict:
    ...
    # access cat instance
    ccat = get_ccat_instance(request)
...
async def install_plugin_from_registry(
    ...
    url = GITHUB_API_BASE +  path_url + "/releases"
    ...
    # Get plugin name
    plugin_name = path_url.split("/")[-1] + ".zip"
